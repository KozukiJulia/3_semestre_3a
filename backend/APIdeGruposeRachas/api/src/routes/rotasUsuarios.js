...
router.post('/usuarios', async (req, res) => {
    ...
    try {
        ...
        const emailExistente = await BD.query(`SELECT * FROM USUARIOS WHERE email = $1`, [email]);
...
router.put('/usuarios/:id_usuario', async (req, res) => {
    ...
    try {
        ...
        const emailExistente = await BD.query(`SELECT * FROM USUARIOS WHERE email = $1 AND id_usuario != $2`, [email, id_usuario]);
        if (emailExistente.rows.length > 0) {
        ...
        id_usuario = $5`;
        const valores = [nome, email, senhaFinal, tipo_acesso, id_usuario];
        await BD.query(comando, valores);
...
router.patch('/usuarios/:id_usuario', async (req, res) => {
    ...
    try {
        ...
        // Verificar se o email já existe para outro usuário, se email foi enviado
        if (email !== undefined) {
            const emailExistente = await BD.query(`SELECT * FROM USUARIOS WHERE email = $1 AND id_usuario != $2`, [email, id_usuario]);
            if (emailExistente.rows.length > 0) {
                return res.status(409).json({ error: 'Email já cadastrado para outro usuário' });
